...
//        jsonObject.put("activeMonthlyUsers", trackUserActivity.getMonthlyActiveUserCount());
        ...
        try {
            // 从文件中读取地图数据
            String filePath = "src/main/resources/data/info.json";
    ...
    public JSONObject getPetTypeProportion() {
       /* List<Map<String, Object>> petTypeCounts = dataVisualMapper.getPetTypeCounts();
        ...
        try {
            // 从文件中读取地图数据
            String filePath = "src/main/resources/data/petProportion.json";
            String jsonContent = new String(Files.readAllBytes(Paths.get(filePath)));
...
//            jsonObject.put("value", count);
    ...
    public List<JSONObject> getTopSellingProducts() {
        ...
        try {
            // 从文件中读取地图数据
            String filePath = "src/main/resources/data/hotProducts.json";
            String jsonContent = new String(Files.readAllBytes(Paths.get(filePath)));
            jsonArray = JSON.parseArray(jsonContent);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public List<JSONObject> getVideoKeywords() {
        ...
        try {
            // 从文件中读取地图数据
            String filePath = "src/main/resources/data/videoTags.json";
            String jsonContent = new String(Files.readAllBytes(Paths.get(filePath)));
            jsonArray = JSON.parseArray(jsonContent);
        } catch (IOException e) {
            e.printStackTrace();
